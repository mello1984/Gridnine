...
class FlightFiltrationAnyMatchImplTest {
    private final FlightFiltration filtration = new FlightFiltrationAnyMatchImpl();
    ...
    @Test
    @DisplayName("return full list when not all predicates are true")
    void getFilteredFlightsList_whenNotAllPredicatesTrue_thenReturnFullList() {
        List<Flight> expected = getFlightsList();
        Predicate<Flight> predicate1 = flight -> true;
        Predicate<Flight> predicate2 = flight -> false;
        var predicates = List.of(predicate1, predicate2);
